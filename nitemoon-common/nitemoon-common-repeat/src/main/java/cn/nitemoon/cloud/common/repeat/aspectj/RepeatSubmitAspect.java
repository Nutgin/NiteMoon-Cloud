package cn.nitemoon.cloud.common.repeat.aspectj;
...
import cn.nitemoon.cloud.common.repeat.annotation.RepeatSubmit;
...
public class RepeatSubmitAspect {
    ...
    public void doBefore(JoinPoint point, RepeatSubmit repeatSubmit) throws Throwable {
        ...
        if (interval < 1000) {
            throw new CommonBusinessException("重复提交间隔时间不能小于'1'秒");
        }
        ...
        Boolean success = redisTemplate.opsForValue().setIfAbsent(cacheRepeatKey, "", Duration.ofMillis(interval));
        if (Boolean.TRUE.equals(success)) {
            KEY_CACHE.set(cacheRepeatKey);
        } else {
            String message = repeatSubmit.message();
            if (StringUtils.startsWith(message, "{") && StringUtils.endsWith(message, "}")) {
                message = SpringUtils.getBean(MessageSource.class).getMessage(StringUtils.substring(message, 1, message.length() - 1), null, LocaleContextHolder.getLocale());
            }
