package cn.nitemoon.cloud.common.storage.core.client.s3;
...
public class S3FileClient extends AbstractFileClient<S3FileClientConfig> {
    ...
    public S3FileClient(Long id, S3FileClientConfig config, Boolean cloudEnable) {
    ...
    public String upload(byte[] content, String path, String type) throws Exception {
        ...
        // 根据bucket类型返回不同的URL
        if (config.getPrivateBucket()) {
            // 私有bucket：返回代理URL，使用后端服务域名
            String backendDomain = config.getBackendDomain();
            if (StrUtil.isEmpty(backendDomain)) {
                // 如果没有配置后端域名，使用domain字段（但这样可能不对）
                backendDomain = config.getDomain();
            }
            return formatFileUrl(backendDomain, path);
        } else {
            // 公开bucket：返回直接URL
     ...
     * @return 文件的内容
     */
    public InputStream getContentAsStream(String path) throws Exception {
