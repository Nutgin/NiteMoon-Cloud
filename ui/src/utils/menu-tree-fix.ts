/**
 * Force fix for menu tree active state in light/dark mode with black theme
 */
export function fixMenuTreeActiveState() {
  ...
  selectedNodes.forEach(node => {
    ...
    if (themeMode === 'light') {
      ...
      // Also apply white text to title and children
      ...
      // Also apply styles to title and children
      const titleElement = element.querySelector('.ant-tree-title');
      if (titleElement) {
...
// Run fix immediately and also observe for changes
if (typeof window !== 'undefined') {
  // Run immediately
  setTimeout(fixMenuTreeActiveState, 100);
  
  // Also run when DOM changes (for dynamic content)
  const observer = new MutationObserver(() => {
    setTimeout(fixMenuTreeActiveState, 50);
  });
